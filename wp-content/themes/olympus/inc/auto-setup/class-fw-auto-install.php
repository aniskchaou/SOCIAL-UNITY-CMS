...
class FW_Simple_Auto_Install {
	...
	public function __construct() {
		$this->file_uri    = get_template_directory_uri() . '/inc/auto-setup';
		$this->file_path   = get_template_directory() . '/inc/auto-setup';
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to activate supported extensions. %s ', 'olympus' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate Unyson dependencies. The %s demo content will not be installed %s.', 'olympus' ), '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'olympus' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the Unyson dependencies manually. Note that this page will not be  accessible until you install the theme again.', 'olympus' )
		);
	}
	...
	private function admin_enqueue_scripts() {
		...
		$auto_setup_data = array(
			...
			'messages'                  => array(
				'on_leave_alert'    => esc_html__( 'Attention, the installation process is not yet finished, if leave this page, you lose the information stored on the site!', 'olympus' ),
				'server_problems'   => esc_html__( "Sorry, we've encountered some errors, try to access this page later.", "olympus" ),
				'process_completed' => esc_html__( 'The installation process was completed successfully.', 'olympus' )
			),
	...
	public function auto_setup_page_view() {
		...
		if ( $this->process_is_running() ) {
			...
			olympus_render($this->render_view( $this->view_path . '/auto_setup.php' ));
	...
	public function render_install_setup( $params ) {
		...
		olympus_render($this->render_view( $this->view_path . '/install_setup.php', $params ));
	}
...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
		...
		if ( ! empty( $this->demos ) ) {
			...
			wp_localize_script(
				...
				array(
					...
					'messages'     => array(
						'installing'              => esc_html__( 'Installing', 'olympus' ),
						'start_install_plugins'   => esc_html__( 'Installing required plugins ...', 'olympus' ),
						'finish_install_plugins'  => esc_html__( 'Finished installing required plugins', 'olympus' ),
						'start_activate_plugins'  => esc_html__( 'Activaiting required plugins ...', 'olympus' ),
						'finish_activate_plugins' => esc_html__( 'Finished activaiting required plugins', 'olympus' ),
					)
				)
			);
		}
	...
	public function get_demo_steps() {
		return array(
			'activate-demo-plugins' => array(
				...
				'message'     => esc_html__( 'Activate required demo plugins', 'olympus' ),
			)
		);
	}
		...
		return array_merge(
			...
			array(
				'activate-unyson'           => array(
					...
					'message'     => esc_html__( 'Activating Unyson', 'olympus' ),
				),
				'activate-required-plugins' => array(
					...
					'message'     => esc_html__( 'Activate required plugins', 'olympus' ),
				)
			)
		);
	}
	...
	public function get_setup_messages() {
		return array(
			'plugins_only'      => sprintf( esc_html__( 'This option will activate %s already installed plugins and dependencies %s by Network Administrator. The %s demo content will not be installed %s.', 'olympus' ), '<b>', '</b>', '<b>', '</b>' ),
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will activate Unyson dependencies together %s with the demo content %s for the theme.', 'olympus' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and activate all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'olympus' )
		);
	}
	...
	public function activate_plugins( $plugins ) {
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return new WP_Error( 0, esc_html__( 'Current user can\'t activate plugins', 'olympus' ) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			return new WP_Error( 0, sprintf(
					esc_html__( 'Failed to activate required plugins. %s', 'olympus' ),
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
	...
	public function get_demo_steps() {
		return array(
			...
			'install-demo-plugins'  => array(
				...
				'message'     => esc_html__( 'Install required demo plugins', 'olympus' ),
			),
		);
	}
	...
	public function install_demo_plugins() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. Folder %s is not writable', 'olympus' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( ! empty( $failed_plugins ) ) {
			...
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install required plugins. %s', 'olympus' ), implode( ', ', $failed_plugins ) ),
			) );
		}
		ob_end_clean();
		wp_send_json_success();
		...
		return array_merge(
			...
			array(
				'install-unyson'               => array(
					...
					'message'     => esc_html__( 'Downloading and installing Unyson', 'olympus' ),
				),
				'install-required-plugins'     => array(
					...
					'message'     => esc_html__( 'Installing required plugins', 'olympus' ),
				),
				'install-supported-extensions' => array(
					...
					'message'     => esc_html__( 'Installing supported extensions', 'olympus' ),
				),
				'install-child-theme'          => array(
					'ajax_action' => $this->_prefix . '_install_child_theme',
					'nonce'       => wp_create_nonce( 'install-child-theme' ),
					'message'     => esc_html__( 'Downloading and installing child theme', 'olympus' ),
				),
			)
		);
	}
	...
	public function install_supported_extensions() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install supported extensions. Folder %s is not writable', 'olympus' ), WP_PLUGIN_DIR ),
			) );
		}
		...
		if ( is_array( $result ) ) {
			...
			if ( ! empty( $failed_extensions ) ) {
				...
				wp_send_json_error( array(
					'message' => sprintf( esc_html__( 'Failed to install supported extensions. %s ', 'olympus' ), implode( ', ', $failed_extensions ) ),
				) );
			}
		}
	...
	public function install_unyson() {
		...
		if ( ! $this->initialize_filesystem( $credentials, WP_PLUGIN_DIR ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Unyson. Folder %s is not writable', 'olympus' ), WP_PLUGIN_DIR ),
			) );
		}
	...
	public function install_child_theme() {
		...
		if ( ! $this->initialize_filesystem( $credentials, wp_normalize_path( get_theme_root() ) ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to install Theme. Folder %s is not writable', 'olympus' ), wp_normalize_path( get_theme_root() ) ),
			) );
		}
	...
	public function get_setup_messages() {
		return array(
			'plugins_and_demo'  => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies together %s with the demo content %s for the theme.', 'olympus' ), '<b>', '</b>' ),
			'plugins_only'      => sprintf( esc_html__( 'This option will install and activate all the plugins and dependencies. The %s demo content will not be installed %s.', 'olympus' ), '<b>', '</b>' ),
			'skip_auto_install' => esc_html__( 'Skip the auto setup all together and install all the plugins manually. Note that this page will not be accessible until you install the theme again.', 'olympus' )
		);
	}
}
...
class FW_Plugin_Installer_Helper extends FW_Installer_Helper {
	...
	public static function install( $plugin, $force = false ) {
		return self::is_installed( $plugin ) && ! $force
			? array(
				'success' => true,
				'data'    => array(
					'message' => esc_html__( 'Plugin is already installed.', 'olympus' )
				)
			)
	...
	public static function activate( $plugin ) {
		if ( ! self::is_installed( $plugin ) ) {
			return array(
				'success' => false,
				'data'    => array(
					'message' => sprintf( esc_html__( 'It was not possible to activate %s. Because it isn\'t installed.', 'olympus' ), $plugin['name'] )
				)
			);
		}
	...
	public static function bulk_activate( $plugins ) {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			return array(
				'message' => sprintf( esc_html__( 'Current user can\'t activate plugins.', 'olympus' ), WP_PLUGIN_DIR ),
			);
		}
	...
	private static function api( $action, $args = null ) {
		...
		if ( false === $res ) {
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', wp_kses( __( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'olympus' ), array( 'a' => array( 'href' => true, 'title' => true ) ) ), $request->get_error_message() );
			} else {
				$res = maybe_unserialize( wp_remote_retrieve_body( $request ) );
				if ( ! is_object( $res ) && ! is_array( $res ) ) {
					$res = new WP_Error( 'plugins_api_failed', wp_kses( __( 'An unexpected error occurred. Something may be wrong with WordPress.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="http://wordpress.org/support/">support forums</a>.', 'olympus' ), array( 'a' => array( 'href' => true, 'title' => true ) ) ), wp_remote_retrieve_body( $request ) );
				}
			}
