	...
	<?php } ?>
		...
		if ( isset( $attr['rtmedia_simple_file_upload'] ) && true === $attr['rtmedia_simple_file_upload'] ) {
			if ( isset( $attr['rtmedia_upload_without_form'] ) && true === $attr['rtmedia_upload_without_form'] ) { ?>
				...
				<form id="rtmedia-uploader-form<?php olympus_render( $main_id );  ?>" method="post" action="upload" enctype="multipart/form-data">
			...
			if ( isset( $attr['rtmedia_upload_allow_multiple'] ) && true === $attr['rtmedia_upload_allow_multiple'] ) {
				?>
				<div class="rtm-file-input-container"><p class="rtmedia-file-upload-p"><input type="file" name="rtmedia_file_multiple[]" multiple="true" class="rtm-simple-file-input" id="rtmedia_simple_file_input<?php olympus_render( $main_id ); ?>" /></p></div><?php } else {
				?>
				<div class="rtm-file-input-container"><p class="rtmedia-file-upload-p"><input type="file" name="rtmedia_file" class="rtm-simple-file-input" id="rtmedia_simple_file_input<?php olympus_render( $main_id ); ?>" /></p></div><?php
			}
			...
			<div class="rtmedia-uploader no-js">
				<div id="rtmedia-uploader-form<?php olympus_render( $main_id ); ?>">
					<?php do_action( 'rtmedia_before_uploader' ); ?>

					<div class="rtm-tab-content-wrapper">
						<div id="rtm-<?php echo esc_attr( $mode ); ?>-ui<?php olympus_render( $main_id ); ?>" class="rtm-tab-content">
							<?php
							do_action( 'rtmedia_before_' . $mode . '_ui' );
							//it is outputting html and make sure content is escaping proper while setting.
							olympus_render( $tabs[ $mode ][ $upload_type ]['content'] ); // @codingStandardsIgnoreLine
						...
						?>
						<input type="submit" id='rtMedia-start-upload<?php olympus_render( $main_id ); ?>' name="rtmedia-upload" value="<?php echo esc_attr( RTMEDIA_UPLOAD_LABEL ); ?>" />
					</div>
				</div>
			<?php
		}
