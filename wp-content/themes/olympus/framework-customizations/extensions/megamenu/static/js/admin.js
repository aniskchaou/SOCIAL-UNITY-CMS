            ...
            if ( localized.icon_option.type === 'icon-v2' ) {
                if ( value ) {
                    ...
                    if ( data['url'] && data.type === "custom-upload" ) {
                        icon.html('');
                        icon.attr( 'class', 'mega-menu-icon-i' ).after( '<img loading="lazy" src="' + data['url'] + '" alt="" />' );
                    } else if ( data['icon-class'] && data.type === "icon-font" ) {
                        var icon_parent = icon.parent();
                        icon.next('img').remove();
                        icon.remove();
                        var icon_el = '<i class="mega-menu-icon-i ' + data['icon-class'] + '"></i>';
                        icon_parent.append(icon_el);
                    } else {
